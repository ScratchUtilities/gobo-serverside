...
function shortenUrl(url) {
	...
	if (split[0] == "discuss") {
		...
		return (
		    "http://gobo.tk/o" + base64.encode(url.split("#")[1].substring(5))
		);
	    }
	} else {
	    id = split[1];
	...
	if (type == "p" && split[2] == "editor") {
	    return "http://gobo.tk/e" + id;
	} else {
	    return "http://gobo.tk/" + type + id;
	}
    } else {
	return "Invalid URL";
    }
...
const base64 = {
	...
	.reverse()
	.reduce(
            (prev, curr, i) => prev + base64.charset.indexOf(curr) * Math.pow(64, i),
            0
	)
};
...
function ready() {
	...
	if (req.url.substring(0, 7) === '/?long=') {
		...
		files['_query']
		    .replace(
			'{input}',
			querystring.unescape(req.url.substring(7))
		    )
		    .replace(
			'{output}',
			shortenUrl(querystring.unescape(req.url.substring(7)))
		    )
	    );
...
load('about/index.html', '/about/', 'text/html');// two versions for with and
load('about.html', '/about', 'text/html');// without slash at end.
